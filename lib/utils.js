...
function getSafeKeypath (obj, keypath) {
  if (obj === null || obj === undefined || typeof keypath !== 'string') {
    return undefined
  }

  var normalizedPath = keypath
    .replace(/\["([^"\]]+)"\]/g, '.$1')
    .replace(/\['([^'\]]+)'\]/g, '.$1')
    .replace(/\[(\d+)\]/g, '.$1')
  ...
  var blockedKeys = ['__proto__', 'prototype', 'constructor']
...
exports.addCheckedFunction = function (env) {
  ...
  env.addGlobal('checked', function (name, value) {
    ...
    var storedValue = getSafeKeypath(this.ctx.data, name)
